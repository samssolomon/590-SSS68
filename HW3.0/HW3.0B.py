...
#------------------------
#DEFINE MODEL (LINEAR, LOGISTIC, OR ANN)
#------------------------

#DEFINE SIGMOID FUNCTION
def S(x): return 1.0/(1.0+np.exp(-x))

#CREATE FUNCTION THAT TAKES A LONG VECTOR W OF WEIGHTS AND BIAS AND RETURNS 
#WEIGHT AND BIAS SUBMATRICES FOR AN ANN MODEL
submatrices=[];
...
#EXTRACT INITIAL LAYER SIZE FROM DATA
if(model_type=="ANN"):
...
#FUNCTION THAT DOES MATRIX MULTIPLICATION WITH ANNS BASED ON SUBMATRICES OF INITIAL GUESS
def NN_eval(x,submatrices):
	act_output=[]
	weights = submatrices[::2] # Extract weights
	bias_terms = submatrices[1::2] # Extract bias terms
	x_input = x; # Initialize x_input
	for i in range(0,len(weights)-1): # Generate input for activation function by multiplying x by weight and adding bias term
		act_input = np.matmul(x_input,weights[i].transpose()) + bias_terms[i].transpose() # Run activation function n-1 times
		if(activation=="SIGMOID"): act_output=S(act_input) # Run input through sigmoid activation function
		if(activation=="TANH"): act_output=np.tanh(act_input) # Run input through tanh activation function
		x_input=act_output # Store output of activation function for next layer of ANN
	final_output=np.matmul(act_output,weights[len(weights)-1].transpose()) + bias_terms[len(bias_terms)-1].transpose() # Obtain final output
	return final_output

#FUNCTION THAT DEFINES MODEL
def model(x,p):
...
#------------------------
#DEFINE LOSS FUNCTION (MSE IN THIS CASE)
#------------------------
...
def minimizer(f,xi):
	...
	while(iteration<=max_iter):
		...
		if(iteration%1==0):
			...
			print("Iteration:",iteration,"	","PARAMETERS:",xi,"	","Training loss:", MSE_T) 
...
print("Number of parameters to be fitted:", NFIT)
po=np.random.uniform(0.25,1.,size=NFIT)
print("Initial guess for parameters:",po)

#TRAIN MODEL USING SCIPY MINIMIZER
