...
autoencoder.save('Convolutional Auto-encoder_MNIST.h5')
...
print("Percent of samples designated as anomalies in MNIST training set: ", 100*np.sum(anomalies1)/x_train.shape[0],"%")


# Load fashion_mnist dataset (images of which are anomalies)
from keras.datasets import fashion_mnist 
(f_x_train, _), (f_x_test, _) = fashion_mnist.load_data()

#-----------RESCALE
f_x_train = f_x_train.astype('float32') / 255.

#-----------RESHAPE
f_x_train = np.reshape(f_x_train, (len(f_x_train), 28, 28, 1))

#-----------GENERATE AUTOENCODER PREDICTIONS FOR Fashion_MNIST
f_x_pred = autoencoder.predict(f_x_train)

#-----------RESHAPE FOR PLOTS
f_x_train=f_x_train.reshape(60000,28,28);
...
f_x_train=f_x_train.reshape(60000,28*28)
f_x_pred=f_x_pred.reshape(60000,28*28)
f_mnist_mse_loss = np.mean((f_x_pred - f_x_train)**2, axis=1)
...
print("Percent of samples designated as anomalies in Fashion MNIST training set: ", 100*np.sum(anomalies2)/f_x_train.shape[0],"%")